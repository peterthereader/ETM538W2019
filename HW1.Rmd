...
In this section, we get the range of a numeric vector by taking the difference between the high and low values from the range output, and if the vector is not numeric, then provide NA. 
Next, we creates the generic structure for the tables in Part B. The variable_class use of *map_chr()* will throw an error on the data-time object because that class has multiple assignments value_type is temporarily NA, reassign one of: "question", "answer", "link".
...
top10_Order_Dollar_byCountry <- inner_join(orders, customers, by = c('cust_id')) %>% 
  ...
  top_n(10)
pander(top10_Order_Dollar_byCountry, caption="Top 10 Countries by Dollar Volume")
```
...
Top10_SellProduct_ByUnit <- inner_join(order_lines, catalog, by = c('product_id'='id')) %>% 
  ...
  top_n(10)
pander(Top10_SellProduct_ByUnit, caption="Top 10 Products by Unit Volume")
...
Top10_SellProduct_ByDollar <- inner_join(order_lines, catalog, by = c('product_id'='id')) %>% 
  ...
  top_n(10)
pander(Top10_SellProduct_ByDollar, caption="Top 10 Products by Dollar Volume")
...
Top5CAbyUnits <- inner_join(order_lines, catalog, by = c('product_id'='id')) %>% 
  ...
  arrange(desc(totUnit)) %>% 
  top_n(5)
pander(Top5CAbyUnits, caption="Top 5 Products by Unit in CA")
```
...
Top5CAbyDollar <- inner_join(order_lines, catalog, by = c('product_id'='id')) %>% 
  ...
  arrange(desc(totDollar)) %>% 
  top_n(5)
pander(Top5CAbyDollar, caption="Top 5 Products by Dollar in CA")
```
...
Top5TXbyUnits <- inner_join(order_lines, catalog, by = c('product_id'='id')) %>% 
  ...
  arrange(desc(totUnit)) %>% 
  top_n(5)
pander(Top5TXbyUnits, caption="Top 5 Products by Unit in TX")
```
...
Top5TXbyDollar <- inner_join(order_lines, catalog, by = c('product_id'='id')) %>% 
  ...
  arrange(desc(totDollar)) %>% 
  top_n(5)
pander(Top5TXbyDollar, caption="Top 5 Products by Dollar in TX")
```
...
Top5CADbyUnits <- inner_join(order_lines, catalog, by = c('product_id'='id')) %>% 
  ...
  arrange(desc(totUnit)) %>% 
  top_n(5)
pander(Top5CADbyUnits, caption="Top 5 Products by Unit in Canada")
```
...
Top5CADbyDollar <- inner_join(order_lines, catalog, by = c('product_id'='id')) %>% 
  ...
  arrange(desc(totDollar)) %>% 
  top_n(5)
pander(Top5CADbyDollar, caption="Top 5 Products by Dollar in Canada")
```
...
Top5SingaporebyUnits <- inner_join(order_lines, catalog, by = c('product_id'='id')) %>% 
  ...
  arrange(desc(totUnit)) %>% 
  top_n(5)
pander(Top5SingaporebyUnits, caption="Top 5 Products by Unit in Singapore")
```
...
Top5SingaporebyDollar <- inner_join(order_lines, catalog, by = c('product_id'='id')) %>% 
  ...
  arrange(desc(totDollar)) %>% 
  top_n(5)
pander(Top5SingaporebyDollar, caption="Top 5 Products by Dollar in Singapore")
```
...
# not in the code: the number of lines in the repeated customers table matches the total of rc$Freq (2219), so there's a working double-check
```
